...
2. macOS Calendar 에서 오늘 (00:00 ~ 다음날 00:00, 로컬 타임존) 시작하는 이벤트 중, **위 필터에 일치하는 캘린더에서 발생하는 이벤트만** 가져온다.
   - 우선순위 1: `icalBuddy` 가 설치되어 있으면 사용. `-ic "<쉼표로 join 한 포함 캘린더 이름>"` 옵션으로 캘린더를 한정.
     예: `icalBuddy -nc -nrd -df "" -tf "%H:%M" -b "" -eep "notes,url,attendees,location" -ic "업무,Work" eventsToday`
   - 우선순위 2: `icalBuddy` 가 없으면 `osascript` 로 `Calendar.app` 조회. 스크립트는 `포함 캘린더` 에 속한 캘린더만 순회하며 `summary|start|end|allday` 한 줄씩 출력. `포함 계정` 이 지정된 경우 그 계정 소속의 모든 캘린더도 함께 포함.
   - 두 방법 모두 실패하면 (권한 거부, 앱 미설치, 필터 불일치 등) → 캘린더 단계만 건너뛰고 워크플로우 계속 진행. step 8 요약에서 "캘린더 수집 실패: <이유>" 보고.
...
- `## TODOs`: 다음 순서로 작성한다.
  ...
  2. **캐리오버 todo**: `### 오늘 일정` 영역 다음에 (또는 일정 영역이 없으면 `## TODOs` 바로 아래에), step 3 에서 수집한 미완료 todo 를 평면 bullet 형태로 삽입. 들여쓰기, 기존 wiki-link 는 모두 **그대로 보존**.
